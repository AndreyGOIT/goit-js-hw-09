...
// запись значений полей инпутов в локал сторадже
const STORAGE_KEY = 'income-data';
...
refs.submit.addEventListener(
  'submit',
  createPromise(refs.inputs[2].value, refs.inputs[0].value)
);
...
function createPromise(position, delay) {
  ...
  for (let position = 1; position <= savedMessage.amount; position += 1) {
